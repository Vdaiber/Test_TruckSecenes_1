...
def main():
    ...
    # NEUER Parameter für den Eingangsfilter
    min_confidence_for_b2_input = float(fusion_cfg.get("min_confidence_for_b2_input", 0.0)) # Standard: 0.0 (kein Filter)
    ...
    all_tracks_from_all_sensors_raw: List[Dict[str, Any]] = [] # Umbenannt für Klarheit
    ...
    # NEU: Filtere Tracks basierend auf min_confidence_for_b2_input
    ...
    # Sortiere die gefilterten Tracks nach Score
    all_tracks_from_all_sensors.sort(key=lambda t: t.get("confidence_score", 0.0), reverse=True)
    
    processed_mask = [False] * len(all_tracks_from_all_sensors) # Maske für die gefilterte Liste
        ...
        if current_fusion_group: 
            for track_in_group in current_fusion_group:
                ...
                if uid_to_find is not None:
                    # Finde den Index in der (potenziell sortierten und gefilterten) Liste 'all_tracks_from_all_sensors'
                ...
                if original_list_idx != -1:
                    ...
                    print(f"WARNUNG B2: Konnte Track aus Gruppe (ID: {track_in_group.get('unique_id_before_fusion')}) nicht in der aktuellen Track-Liste zum Markieren finden.")
