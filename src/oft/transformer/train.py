...
def load_checkpoint(checkpoint_path, model_or_encoder, decoder, optimizer, lr_scheduler, device, logger): # Angepasst für model oder encoder/decoder
    """
    Lädt einen Checkpoint für das Modell, den Optimizer und den LR-Scheduler.
    Args:
        checkpoint_path (str): Pfad zur Checkpoint-Datei.
        model_or_encoder (torch.nn.Module): Das Gesamtmodell oder der Encoder.
        decoder (Optional[torch.nn.Module]): Der Decoder, falls separat vom Encoder geladen.
        optimizer (torch.optim.Optimizer): Der Optimizer.
        lr_scheduler: Der Lernraten-Scheduler.
        device (torch.device): Das Gerät, auf das der Checkpoint geladen wird.
        logger (logging.Logger): Logger für Ausgaben.
    Returns:
        tuple: (start_epoch, best_metric_val)
    """
    ...
    try:
        ...
        if isinstance(model_or_encoder, ObjectFusionTransformerModel):
            # Lade Encoder und Decoder des Gesamtmodells
            ...
            # Lade separate Encoder und Decoder
            ...
            logger.info("Separate Encoder and Decoder states loaded.")
        else:
...
def save_checkpoint(state, is_best, checkpoint_dir, filename_prefix="checkpoint", specific_epoch=None):
    ...
    Args:
        state (dict): Dictionary mit dem Zustand (Modell, Optimizer, Epoche etc.).
        is_best (bool): Ob dieser Checkpoint der bisher beste ist.
        checkpoint_dir (str): Verzeichnis zum Speichern der Checkpoints.
        filename_prefix (str): Präfix für die Checkpoint-Dateinamen.
        specific_epoch (Optional[int]): Wenn angegeben, wird ein Checkpoint für diese spezifische Epoche gespeichert.
    """
    ...
    logging.info(f"Checkpoint saved to: {filepath}") # Verwende logging statt print
    if is_best:
        best_filepath = os.path.join(checkpoint_dir, f"{filename_prefix}_best.pth.tar")
        torch.save(state, best_filepath)
        logging.info(f"Best checkpoint updated and saved to: {best_filepath}") # Verwende logging
# --- ENDE CHECKPOINT-FUNKTIONEN ---
...
class ObjectFusionTransformerModel(torch.nn.Module):
    ...
    def forward(self,
        ...
        """
        Forward-Pass des Gesamtmodells.
        Args:
            encoder_input_features (torch.Tensor): Eingabe-Features für den Encoder.
            encoder_input_xyz_centers (torch.Tensor): XYZ-Zentren für Positional Encoding im Encoder.
            encoder_input_mask (Optional[torch.Tensor]): Padding-Maske für den Encoder-Input.
        Returns:
            Dict[str, torch.Tensor]: Dictionary mit den Vorhersagen des Decoders ('pred_logits', 'pred_boxes').
        """
        memory = self.encoder(
        ...
        predictions = self.decoder(
            memory=memory,
            memory_key_padding_mask=encoder_input_mask # Die gleiche Maske kann für die Memory verwendet werden
        )
        return predictions
...
def train_one_epoch(model: ObjectFusionTransformerModel,
                    ...
                    device: torch.device,
                    epoch: int,
                    cfg_training: DictConfig, # Dies ist cfg.training
                    logger: logging.Logger):
    """
    Trainiert das Modell für eine Epoche.
    Args:
        model (ObjectFusionTransformerModel): Das zu trainierende Modell.
        criterion (SetCriterion): Die Verlustfunktion.
        data_loader (DataLoader): DataLoader für die Trainingsdaten.
        optimizer (torch.optim.Optimizer): Der Optimizer.
        device (torch.device): Das Gerät für das Training.
        epoch (int): Die aktuelle Epochennummer.
        cfg_training (DictConfig): Konfiguration für das Training (cfg.training).
        logger (logging.Logger): Logger für Ausgaben.
    Returns:
        dict: Dictionary mit den durchschnittlichen Verlustwerten der Epoche.
    """
    model.train()
    criterion.train()
    ...
    for batch_idx, batch_dict in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if losses_dict_unweighted: # Stelle sicher, dass das Dict nicht leer ist
            for loss_name, loss_val_unweighted in losses_dict_unweighted.items():
                weight = criterion.weight_dict.get(loss_name, 1.0) # Default-Gewicht 1.0, falls nicht im Dict
        ...
        metric_logger.update(loss=total_loss.item()) # Gesamtverlust (gewichtet)

        optimizer.zero_grad()
        if torch.is_tensor(total_loss) and total_loss.requires_grad: # Zusätzliche Prüfung
            total_loss.backward()
...
def evaluate_model_internally(model: ObjectFusionTransformerModel,
                              ...
                              epoch: int):
    """
    Evaluiert das Modell intern (Verlustberechnung) und bereitet Vorhersagen für das DevKit vor.
    Args:
        model (ObjectFusionTransformerModel): Das zu evaluierende Modell.
        criterion (SetCriterion): Die Verlustfunktion.
        data_loader (DataLoader): DataLoader für die Validierungsdaten.
        device (torch.device): Das Gerät für die Evaluation.
        cfg (DictConfig): Die vollständige Hydra-Konfiguration.
        logger (logging.Logger): Logger für Ausgaben.
        epoch (int): Die aktuelle Epochennummer.
    Returns:
        tuple: (val_loss_stats, predictions_for_devkit, gt_detections_for_devkit, sample_tokens_for_devkit)
    """
    model.eval()
    criterion.eval()
    ...
    print_freq = cfg.training.print_freq_val # Eigene Print-Frequenz für Validierung

    all_predictions_for_devkit = []
    all_gt_detections_raw_for_devkit = [] # Für die DevKit Evaluation
    all_sample_tokens_for_devkit = []     # Für die DevKit Evaluation
    ...
    for batch_idx, batch_dict in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        gt_detections_list_raw_batch = batch_dict['gt_detections_list_raw'] # Rohe GTs für DevKit
        ...
        if losses_dict_unweighted:
            for loss_name, loss_val_unweighted in losses_dict_unweighted.items():
                weight = cfg.loss.loss_weight_dict.get(loss_name, 1.0) # cfg.loss statt criterion
                total_loss += loss_val_unweighted * weight
                # Logge nur die Hauptverluste für die Validierung, um es übersichtlich zu halten
                if loss_name in ['loss_ce', 'loss_bbox_l1', 'loss_giou']: # Oder die gewichteten Namen
                     metric_logger.update(**{loss_name: (loss_val_unweighted * weight).item()})
        metric_logger.update(loss=total_loss.item()) # Gesamtverlust (gewichtet)

        # Konvertiere Vorhersagen ins DevKit-Format
        batch_size = predictions['pred_logits'].shape[0]
        for i in range(batch_size):
            sample_pred_logits = predictions['pred_logits'][i] # (NumQueries, NumClasses+1)
            sample_pred_boxes = predictions['pred_boxes'][i]   # (NumQueries, BoxDim)

            sample_output_boxes = [] # Für dieses eine Sample

            scores_all_classes = F.softmax(sample_pred_logits, dim=-1) # (NumQueries, NumClasses+1)
            # Nimm Scores und Indizes für alle Klassen außer der Hintergrundklasse
            pred_scores, pred_labels_indices = torch.max(scores_all_classes[:, :-1], dim=-1) # (NumQueries)

            score_threshold = cfg.evaluation.eval_detection_cfg.get("conf_th_eval", 0.01) # Aus Config

            for q_idx in range(sample_pred_logits.shape[0]): # Iteriere über Queries
                if pred_scores[q_idx] > score_threshold:
                    center = sample_pred_boxes[q_idx, 0:3].cpu().tolist()
                    size = sample_pred_boxes[q_idx, 3:6].cpu().tolist() # w, l, h
                    ...
                    # cfg.dataset.class_names enthält jetzt direkt die 12 DevKit-Namen
                    class_names_list = OmegaConf.to_container(cfg.dataset.class_names, resolve=True)
                    
                    # Der `detailed_detection_name` ist jetzt direkt ein DevKit-Name (oder sollte es sein)
                    # wenn der Index gültig ist.
                    if label_idx < len(class_names_list):
                        mapped_detection_name = class_names_list[label_idx]
                    else:
                        # Dieser Fall sollte seltener auftreten, da num_classes im Modell = 12 ist.
                        logger.warning(f"Predicted label_idx {label_idx} is out of bounds for class_names_list (len {len(class_names_list)}). Setting to 'unknown'.")
                        mapped_detection_name = "unknown"

                    # --- KORREKTUR/VEREINFACHUNG DES MAPPINGS ---
                    # Da cfg.dataset.class_names jetzt die DevKit-Namen sind, ist das Mapping einfacher.
                    # Wir müssen nur sicherstellen, dass der Name gültig ist.
                    if mapped_detection_name not in TRUCKSCENES_DETECTION_NAMES:
                        # Wenn der Name aus class_names_list (sollte ein DevKit-Name sein)
                        # trotzdem nicht in TRUCKSCENES_DETECTION_NAMES ist (sollte nicht passieren bei korrekter Config)
                        # oder wenn label_idx ungültig war und zu "unknown" führte.
                        logger.debug(f"Skipping prediction for devkit: original_class_from_model_output='{mapped_detection_name}', score={pred_scores[q_idx].item()}. Not a valid TRUCKSCENES_DETECTION_NAME or was 'unknown'.")
                        continue
                    # --- ENDE KORREKTUR/VEREINFACHUNG ---
                    ...
                    det_box = DetectionBox(
                        ...
                        velocity=[0.0, 0.0], # Platzhalter, da nicht direkt vorhergesagt
                        ego_translation=[0.0,0.0,0.0], # Platzhalter
                        num_pts=-1, # Platzhalter
                        detection_name=mapped_detection_name,
                        detection_score=pred_scores[q_idx].item(),
                        attribute_name="" # Platzhalter, Attribute werden hier nicht evaluiert
                    )
            ...
            # Füge rohe GTs für dieses Sample hinzu
            if isinstance(gt_detections_list_raw_batch[i], list): # Stelle sicher, dass es eine Liste ist
    ...
    # Erstelle das finale Dictionary für die DevKit Submission
    # Meta-Informationen
    default_devkit_meta = {"use_lidar": True, "use_camera": False, "use_radar": False, "use_map": False, "use_external": False}
    devkit_meta_node = cfg.evaluation.get("devkit_meta")
    ...
    for item in all_predictions_for_devkit: # all_predictions_for_devkit enthält jetzt pro Sample ein Dict
...
def evaluate_model_with_devkit(cfg: DictConfig,
                               prediction_submission_dict: Dict, # Das ist jetzt das "final_submission_dict"
                               devkit_dataroot: str,
                               devkit_version: str,
                               devkit_eval_split: str, # z.B. "mini_val"
                               output_dir_epoch_eval: str, # Verzeichnis für diese Epoche
                               logger: logging.Logger,
                               current_epoch: int): # Für Dateinamen
    """
    Evaluiert die Modellvorhersagen mit dem TruckScenes DevKit.
    Args:
        cfg (DictConfig): Die vollständige Hydra-Konfiguration.
        prediction_submission_dict (Dict): Das finale Dictionary im DevKit Submissionsformat.
        devkit_dataroot (str): Pfad zum Root-Verzeichnis des Datensatzes für das DevKit.
        devkit_version (str): Version des Datensatzes für das DevKit.
        devkit_eval_split (str): Name des Splits für die DevKit-Evaluation.
        output_dir_epoch_eval (str): Verzeichnis, in dem die Evaluationsergebnisse dieser Epoche gespeichert werden.
        logger (logging.Logger): Logger für Ausgaben.
        current_epoch (int): Aktuelle Epochennummer (für Dateinamen).
    Returns:
        Optional[Dict]: Zusammenfassung der DevKit-Metriken oder None bei Fehlern.
    """
    ...
    # Speichere die Vorhersagen im JSON-Format für das DevKit
    ...
    # Lade Konfiguration für DetectionEval aus der Haupt-YAML
    eval_cfg_yaml = cfg.evaluation.eval_detection_cfg # Sollte ein DictConfig-Objekt sein

    # --- Erstelle DetectionConfig für das DevKit ---
    ...
    if isinstance(class_range_from_yaml, (DictConfig, dict)):
        ...
        if missing_keys:
            logger.warning(f"DevkitDetectionConfig: 'class_range' from YAML is a dictionary, but is missing keys: {missing_keys}. Using default value 50 for missing keys.")
...
class MetricLogger(object):
    def __init__(self, delimiter="\t", logger=None):
        ...
        self.logger = logger if logger else logging.getLogger("default_metric_logger") # Default Logger
    ...
    def add_meter(self, name: str, meter: SmoothedValue): # Type Hinting hinzugefügt
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        except TypeError: iterable_len = -1 # Falls iterable keinen __len__ hat
        ...
        for obj in iterable:
            ...
            yield obj # Gebe das Objekt zurück, damit die Schleife weiterlaufen kann
        ...
        avg_time_per_it = total_time / i if i > 0 else 0 # Verhindere Division durch Null
...
def main(cfg: DictConfig):
    ...
    logger = logging.getLogger("train_script_logger") # Konsistenter Logger-Name
    logger.setLevel(logging.INFO) # Oder cfg.log_level
    if not logger.handlers: # Verhindere doppelte Handler bei mehrmaligem Aufruf (z.B. in Notebooks)
    ...
    seed = cfg.training.get("seed", 42) # Default-Seed, falls nicht in Config
    torch.manual_seed(seed)
    np.random.seed(seed)
    random.seed(seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(seed)
        # torch.backends.cudnn.deterministic = True # Kann Performance kosten
        # torch.backends.cudnn.benchmark = False    # Kann Performance kosten
    ...
    dataset_train = ObjectFusionGTDataset(
        ...
        pipeline_config=OmegaConf.to_container(cfg, resolve=True), # Übergebe als Dict
        verbose=True # Für detaillierte Infos beim Initialisieren
    )
    dataset_val = ObjectFusionGTDataset(
        dataroot=cfg.dataset.dataroot, version=cfg.dataset.version,
        split_name=cfg.training.val_split_name,
        pipeline_config=OmegaConf.to_container(cfg, resolve=True), # Übergebe als Dict
        verbose=False # Weniger Output für Validierung
    )
    ...
    dataloader_train = DataLoader(
        ...
        pin_memory=device.type == 'cuda' # Pin Memory nur für CUDA
    ) if sampler_train else None

    dataloader_val = DataLoader(
        dataset_val, batch_size=cfg.training.batch_size, sampler=sampler_val, # Val-Batchsize ggf. anpassen
        collate_fn=object_fusion_gt_collate_fn, num_workers=cfg.training.num_workers,
    ...
    if not dataloader_train and cfg.training.epochs > 0 : # Nur Fehler, wenn Training geplant ist
    ...
    if cfg.training.use_dataparallel and torch.cuda.device_count() > 1:
        ...
        model = torch.nn.DataParallel(model) # model.module für Zugriff auf Originalmodell

    # --- Verlustfunktion und Matcher ---
    ...
    criterion = SetCriterion(
        num_classes=cfg.model.num_classes, # Wird jetzt 12 sein, basierend auf der neuen YAML
        matcher=matcher,
        ...
        coord_normalization_factor=cfg.model.pe_max_coord_val 
    ).to(device)

    # --- Optimizer und LR Scheduler ---
    ...
    start_epoch = 0
    best_metric_val = 0.0 
    checkpoint_dir_path = Path(cfg.training.checkpoint_dir) # Wird jetzt z.B. ./checkpoints_oft_c_pipeline_12cls
